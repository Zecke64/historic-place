...
window.onload = function () {
	...
	map.addControl (new OpenLayers.Control.Tooltip ({
		...
		//--------------------------------------------------------------
		//	Basisposition fuer die Messung
		//--------------------------------------------------------------

		saveLonLat: null,

		//--------------------------------------------------------------
		//	Sobald die Karte bekannt ist, installiere Click-Handler
		//--------------------------------------------------------------

		setMap: function() {

			OpenLayers.Control.Tooltip.prototype.setMap.apply(this,arguments);
			this.map.events.register ('click', this, this.onclick);
		},

		//--------------------------------------------------------------
		//	Der Event-handler onclick(ev) aktiviert den Tooltip.
		//	Beim zweiten Klick wird die Position eingefroren.
		//--------------------------------------------------------------
		...
		onclick: function (ev) {
			...
			if (this.saveLonLat) {
				...
				this.saveLonLat = null;
				this.deactivate();
				this.div.style.display='none';

			} else if (this.active) {

				this.saveLonLat=this.map.getLonLatFromViewPortPx(ev.xy).
				...
				input.id = 'select';
				input.value = value;
				input.setAttribute('length', value.length);
		...
		//--------------------------------------------------------------
		//	Erzeuge den Inhalt des Tooltips
		//--------------------------------------------------------------
		...
		getContent: function (ev) {
			...
			return OpenLayers.Control.MousePosition.prototype.formatOutput (lonLat);
		}
	}));
	//----------------------------------------------------------------------------
	//	Download GPX
