//------------------------------------------------------------------------------
//	$Id: popupmarker.js,v 1.129 2014/10/19 12:20:01 wolf Exp wolf $
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//
//	Revision 1.126  2014/08/23 15:30:49  wolf
//	*** tooltip size
//
...
OpenLayers.Layer.PopupMarker = OpenLayers.Class(OpenLayers.Layer.Markers,{
	...
	setTooltip: function (marker) {
		...
		if (this.maxTooltipWidth !== null) {
			...
			tooltipDiv.style.height  = (size.h+1) + 'px';
		}
	...
	formatValue: function (text, tag) {
		...
		if (!(text+'').match(/http:/)) {
			...
			var tag_match = tag.match(/^(.*:)?wikipedia(:(\w\w))?$/);

			if (tag_match) {

				var lcode = tag_match[3];
				var lemma = text;

				if (!lcode) {

					var text_match = (text+'').match(/^(\w\w):(.+)$/);
					if (text_match) {
				...
				if (lcode) {
					...
					return '<a target="_blank" href="http://' +
						lcode + '.wikipedia.org/wiki/' +
						this.html(lemma.split('\040').join('_')) +
						'">' + this.html(text) + '</a>';
				}
			}

			if (tag.match(/^(.*:)?wikidata$/)) {

				var vals = (text+'').split(/\s*;\s*/);
				var list = [];
				for (var i in vals) {

					var val = vals[i];

					if (!val.match(/^Q[1-9]\d*$/)) {

						list.push(val);
						continue;
					}
					list.push('<a target="_blank" href="https://www.wikidata.org/wiki/' +
						val + '">' + val + '</a>');
				}
				return list.join('; ');
			}

			if (tag.match(/^(image|image:\w+)$/)) {
