	...
	createLayer: function (filename, text) {
		...
		for (var i in features) {
			...
			case 'OpenLayers.Geometry.Point':
				...
				feature.style = OpenLayers.Util.extend(null, this.pointStyle);
				break;

			default:

				// feature.style = OpenLayers.Util.extend(this.trackStyle, null);
				feature.style = OpenLayers.Util.extend(null, this.trackStyle);
				break;
			}
			...
			if (feature.attributes && feature.attributes.desc) {

				titles.push("--- ");
				titles.push(feature.attributes.desc);
			}
		...
		var ctlLayers = (hoverCtrl.layers) ? hoverCtrl.layers : [];
		ctlLayers.push(layer);
		hoverCtrl.setLayer (ctlLayers);
		//hoverCtrl.activate();
		selectCtrl.setLayer (ctlLayers);

		var markers = this.map.getLayersBy("permaId", 'He')[0];
		alert ('markers: '+this.map.getLayerIndex(markers));
		this.map.setLayerIndex(markers, this.map.layers.length-1);
		alert ("Popups: "+this.map.getLayerIndex(markers).toString()+"\ntrack: "+this.map.getLayerIndex(layer).toString());
