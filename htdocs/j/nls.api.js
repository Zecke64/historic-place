...
function NLSTileUrlOS( x, y, z ) {
	...
	if (x == undefined) {
		return "https://t%2."+tileserver_default+'%4.jpg';
	...
	if (x == "${x}") {
		...
		if (tileserver == "")
			for (no=0;no<5;no++)
				urls.push("https://t"+no+"."+tileserver_default+z+'/'+x+'/'+y+'.jpg');
		else
			for (no=0;no<5;no++)
				urls.push("https://t"+no+"."+tileserver+z+'/'+x+'/'+y+'.jpg');
		return urls;
	}
  ...
  var no = (x+y) % 4;
  return "https://nls-"+no+".tileserver.com/nls/"+z+'/'+x+'/'+y+'.jpg';

	// with numbers let's return directly the url to the tile on the server
	var no = (x+y) % 5;
	if (tileserver == "") return "https://t"+no+"."+tileserver_default+z+'/'+x+'/'+y+'.jpg';
	else return "https://t"+no+"."+tileserver+z+'/'+x+'/'+y+'.jpg';
}
...
//
// We folks at historic.place decided we do not want us Google tracked,
// so we comment out this cookie stuff
//
